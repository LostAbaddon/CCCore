...
class Server {
	constructor(config, managers) {
		...
		this.server = null;
		this.sessions = {};
		this.activeWorkers = {}; // 记录活跃的 worker，key 为 tabId
	}
	...
	async handleRequest(req, res) {
		...
		try {
			...
			if (pathname === '/api/config/workspaces' && req.method === 'GET') {
				await this.handleGetWorkspacesConfig(req, res);
				return;
			}

			if (pathname === '/api/config/workspaces' && req.method === 'POST') {
				await this.handleSetWorkspacesConfig(req, res);
				return;
			}
			...
			// 处理 /claudius/:sessionId/submit|clear|interrupt 的 POST
			const claudiusMatch = pathname.match(/^\/claudius\/([^/]+)\/(submit|clear|interrupt)$/);
			if (claudiusMatch && claudiusMatch[1] && req.method === 'POST') {
				...
				else if (claudiusMatch[2] === 'interrupt') {
					await this.handleClaudiusInterrupt(req, res, tabId);
					return;
				}
			}
	 ...
	 * 获取工作区配置
	 */
	async handleGetWorkspacesConfig(req, res) {
		try {
			const config = this.managers.configManager.getWorkspacesConfig();
			res.writeHead(200);
			res.end(JSON.stringify({ ok: true, data: config }));
		}
		catch (error) {
	 ...
	 * 设置工作区配置
	 */
	async handleSetWorkspacesConfig(req, res) {
		let body = '';
		req.on('data', (chunk) => {
			body += chunk;
		});
		...
		req.on('end', async () => {
			try {
				const { searchWorkspace, writingWorkspace } = JSON.parse(body);
				const config = await this.managers.configManager.setWorkspacesConfig(
					searchWorkspace,
					writingWorkspace
				);
			...
			try {
				...
				try {
					...
					const worker = new Worker(path.resolve(__dirname, 'claudeCodeWorker.js'), {
						workerData: { workDir, prompt, model, resumeSid: session.id }
					});

					// 保存 worker 引用
					this.activeWorkers[tabId] = worker;
					...
					// worker 完成后清理
					if (this.activeWorkers[tabId]) {
						worker.terminate();
						delete this.activeWorkers[tabId];
					}
					...
					// 清理 worker
					if (this.activeWorkers[tabId]) {
						this.activeWorkers[tabId].terminate();
						delete this.activeWorkers[tabId];
					}
	 ...
	 * 处理 Claudius 执行中断
	 */
	async handleClaudiusInterrupt(req, res, tabId) {
		try {
			// 检查是否有活跃的 worker
			const worker = this.activeWorkers[tabId];
			...
			// 终止 worker
			console.log('[Server] 中断 tabId:', tabId);
			worker.terminate();
			delete this.activeWorkers[tabId];
			...
			console.error('[Server] Claudius 中断失败:', error);
			res.writeHead(500);
			res.end(JSON.stringify({ ok: false, error: error.message }));
		}
	}

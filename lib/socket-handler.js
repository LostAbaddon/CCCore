...
class SocketHandler {
	...
	async commandCreateReminder(data) {
		...
		const triggerTime = data.triggerTime || Date.now();
		data.triggerTime = triggerTime;

		// 检测远端是否可用
		try {
			const status = await this.managers.extManager.checkSessionFocusStatus(sessionId);
			console.log("[CreateReminder] Get Reminder Info", status);
			// 如果浏览器端不可用
			if (!status || !status.ok) {
				console.warn('[CreateReminder] Connect Claudius failed.');
				return { ok: false, fallback: true };
			}

			// 创建延迟提醒
			const reminder = this.managers.reminderManager.addReminder(data);
			const delay = Math.max(0, triggerTime - Date.now());
			this.managers.reminderManager.localReminders[reminder.id] = setTimeout(async () => {
				console.log('[CreateRemidner] Start Remidner', reminder);
				await this.managers.reminderManager.createReminder(reminder);
				console.log('[CreateRemidner] Remidner Finish', reminder);
			}, delay);
		}
		catch (err) {
			console.error('[CreateReminder] Create Reminder Failed.', err);
	...
	async commandSendReminder(data) {
		...
		return this.commandCreateReminder(data);
	}

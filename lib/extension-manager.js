...
class ExtensionManager {
	 ...
	 * @param {string} [reminder.sessionId] 会话 ID（可选）
	 * @returns {Object} 结果对象
	 */
	...
	async checkSessionFocusStatus(sessionId) {
		try {
			// 智能判断是否需要发送提醒
			const checkResult = await this.wsManager.sendToExtension({
				action: 'CHECK_REMINDER_NEEDED',
				data: { sessionId },
			});

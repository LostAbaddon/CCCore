...
const log = (level, ...msg) => {
	const tag = '[Worker:' + process.pid + ':' + threadId + ']';
	(console[level || 'log']	|| console.log)(tag, ...msg);
	if (!IS_DEV) return;

	const timestamp = new Date().toISOString();
	msg = msg.map(item => JSON.stringify(item)).join(', ');
	const logMessage = `[${timestamp}] [${tag}] ${msg}\n`;
...
if (!result.stdout) {
	...
	try {
		const json = JSON.parse((result.stdout.match(/\{[\w\W]+\}/i) || [])[0] || '{is_error: true}');
		log('log', 'Claude Reply JSON:', json);
		if (json.is_error) {
			log('error', 'Claude Parse Failed:', result.stderr || "ClaudeCode 返回出错");
		...
		log('error', 'Claude Reply Failed:', err.message);
		throwError("ClaudeCode 返回结果解析出错");
	}
}

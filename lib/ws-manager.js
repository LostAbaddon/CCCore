...
class WSManager {
	...
	handlePageInfo(pageInfo) {
		// 这里可以将页面信息转发给日志管理器
		console.log(pageInfo.timestamp, typeof pageInfo.timestamp);
		this.managers.loggerManager.addLog({
			source: "Chrome",
			tabId: pageInfo.tabId,
			timestamp: pageInfo.timestamp,
			content: `打开网页: [${pageInfo.title || '(UNTITLED)'}](${pageInfo.url || 'http://unknown_url'})`
		});
	}
	/**
	 * 向 Extension 发送消息
